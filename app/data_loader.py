...
REQUIRED_ODDS_COLS = {"Team", "Win WC", "Win Div", "Win Conf"}  # keep minimal; expand later
...
def canonicalize_pts(pts_df: pd.DataFrame) -> pd.DataFrame:
    """Standardize pts table to canonical column names: team, unit."""
    out = pts_df.copy()
    out["team"] = _normalize_team(out["team"])
    out = out.rename(columns={"position": "unit"})
    return out


def canonicalize_odds(odds_df: pd.DataFrame) -> pd.DataFrame:
    """Standardize odds table to canonical column names: team, plus odds cols."""
    out = odds_df.copy()
...
def join_pts_with_odds(pts_c: pd.DataFrame, odds_c: pd.DataFrame) -> pd.DataFrame:
    """
    Inner-join odds onto team×unit rows, restricting to teams present in playoff odds.
    Expected rows: (#odds teams) * 6 units (assuming complete pts coverage).
    """
    merged = pts_c.merge(
        odds_c,
        on="team",
